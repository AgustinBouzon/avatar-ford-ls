...
const kioskPhonePreset = normalizePhoneInput(
  process.env.NEXT_PUBLIC_KIOSK_PHONE
)
...
const FEATURE_TILES: FeatureTile[] = [
  ...
  },
  {
    ...
    description: "Procesamos tu foto y la enviamos directo a tu WhatsApp.",
    icon: Send,
...
const FLOW_STEPS: FlowStep[] = [
  ...
  },
  {
    ...
    highlight: "Linky te escucha en vivo y conversa como un asesor Ford.",
    icon: MessageCircle,
...
export function ConvAI({ variant = "default" }: ConvAIProps) {
  ...
  const [captureCountdown, setCaptureCountdown] = React.useState<number | null>(
    null
  )
  ...
  const triggerAutoCapture = React.useCallback(
    async (source: "auto" | "manual", instructions?: string) => {
      ...
      if (!isPhoneValid) {
        setCaptureStatus("error")
        setCaptureError("Ingresa un WhatsApp válido con código de país.")
        captureInFlightRef.current = false
        return
      }
      ...
      try {
        ...
        if (
          ...
          setCameraMessage(
            "Necesitamos acceso a tu cámara. Toca 'Activar cámara' y acepta el permiso."
          )
          setCaptureError(null)
    ...
    clientTools: {
      capture: async (parameters: { instructions?: string }) => {
        ...
        setCaptureInfo({
          ...
          instructions:
            parameters?.instructions || "El agente solicito capturar la foto",
  ...
  const cameraButtonLabel =
    captureStatus === "permission" || !isCameraVisible
      ? "Activar camara"
      : "Ocultar camara"
    ...
    if (!isCameraVisible || captureStatus === "permission") {
      ...
      try {
        ...
        if (
          ...
          setCaptureStatus("error")
          setCaptureError(
            "No pudimos activar la camara. Revisa los permisos del navegador."
          )
        }
        captureInFlightRef.current = false
        return
  ...
  const progressValue = ((currentStepIndex + 1) / FLOW_STEPS.length) * 100
  if (isSimple) {
    return (
        ...
        <div className='flex flex-col gap-5 items-center'>
          <div className='rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-sky-100/80 w-full max-w-md'>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div className='text-center'>
                ...
                <p className='mt-2 text-base text-sky-50'>
                  {captureStatusLabel}
                </p>
              </div>
            </div>
          ...
          <div className='space-y-2 w-full max-w-md'>
            ...
            <Input
              ...
              onFocus={() => {
                if (!phoneNumber) setPhoneNumber("+54 9")
              }}
            ...
            {phoneTouched && !isPhoneValid ? (
              <p className='text-xs text-rose-200/80'>
                Ingresá tu WhatsApp con código internacional (ej: +54 9 11 1234
                5678).
              </p>
            ) : null}
          </div>
          ...
          <div className='rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-sky-100/75 max-w-md'>
            Tip: Pedile a Linky que te cuente algo sobre Ford y después pedile
            que te saque la foto. Apenas termine, la selfie llega directo a tu
            WhatsApp.
          </div>
        </div>
              ...
              <div className='flex flex-col gap-5'>
                <div className='rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur'>
                  <div className='flex w-full flex-col items-center gap-4'>
                    ...
                    {captureInfo ? (
                      <div className='w-full max-w-[320px] rounded-2xl border border-white/15 bg-white/5 p-3 text-xs text-sky-100/85'>
                        ...
                        <div className='mt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] text-sky-100/60'>
                          <span>
                            Motivo:{" "}
                            {captureInfo.reason ?? "Indicacion del Voicebot"}
                          </span>
                          {typeof captureInfo.confidence === "number" ? (
                            <span>
                              Confianza{" "}
                              {Math.round(captureInfo.confidence * 100)}%
                            </span>
                          ) : null}
                        </div>
                      </div>
                    ...
                    {cameraMessage ? (
                      <p className='max-w-[320px] text-center text-xs text-sky-100/70'>
                        {cameraMessage}
                      </p>
                    ) : null}
                  </div>
                ...
                <div className='space-y-3'>
                  ...
                  {phoneTouched && !isPhoneValid ? (
                    <p className='text-xs font-semibold text-rose-200'>
                      Ingresá un número válido con código de país (ej: +54 9 11
                      1234 5678).
                    </p>
                  ) : null}
                </div>
          ...
          <div className='flex items-center gap-6'>
            <div className='space-y-3 flex-1'>
              ...
              <p className='max-w-2xl text-base text-sky-100/80 leading-relaxed'>
                Ingresá tu número y hablá con nuestro voicebot de Ford. Podés
                preguntarle algún dato interesante de la marca o pedirle que te
                saque una foto. ¡En pocos minutos, te enviaremos una versión
                animada de tu foto por WhatsApp!
              </p>
            </div>
          </div>
